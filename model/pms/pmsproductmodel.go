...
type (
	...
	PmsProduct struct {
		...
		FreightTemplateId          int64     `db:"freight_template_id"`
		...
		ServiceIds                 string    `db:"service_ids"`     // 以逗号分割的商品服务: 1->无忧退货; 2->快速退款; 3->免费包邮
		Keywords                   string    `db:"keywords"`
		Note                       string    `db:"note"`
		AlbumPics                  string    `db:"album_pics"` // 画册图片, 连商品图片限制为5张, 以逗号分割
		DetailTitle                string    `db:"detail_title"`
		DetailDesc                 string    `db:"detail_desc"`
		DetailHtml                 string    `db:"detail_html"`           // 商品详情网页内容
...
func (m *PmsProductModel) Insert(data PmsProduct) (sql.Result, error) {
	...
	ret, err := m.conn.Exec(query, data.BrandId, data.ProductCategoryId, data.FreightTemplateId, data.ProductAttributeCategoryId, data.Name, data.Pic, data.ProductSn, data.DeleteStatus, data.PublishStatus, data.NewStatus, data.RecommendStatus, data.VerifyStatus, data.Sort, data.Sale, data.Price, data.PromotionPrice, data.GiftGrowth, data.GiftPoint, data.UsePointLimit, data.SubTitle, data.Description, data.OriginalPrice, data.Stock, data.LowStock, data.Unit, data.Weight, data.PreviewStatus, data.ServiceIds, data.Keywords, data.Note, data.AlbumPics, data.DetailTitle, data.DetailDesc, data.DetailHtml, data.DetailMobileHtml, data.PromotionStartTime, data.PromotionEndTime, data.PromotionPerLimit, data.PromotionType, data.BrandName, data.ProductCategoryName)
	return ret, err
}
