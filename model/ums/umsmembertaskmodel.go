...
var (
	umsMemberTaskFieldNames          = builderx.RawFieldNames(&UmsMemberTask{})
...
type (
	...
	UmsMemberTask struct {
		...
		Integration int64  `db:"integration"` // 赠送积分
...
func (m *UmsMemberTaskModel) Insert(data UmsMemberTask) (sql.Result, error) {
	...
	ret, err := m.conn.Exec(query, data.Name, data.Growth, data.Integration, data.Type)
	return ret, err
}
...
func (m *UmsMemberTaskModel) Update(data UmsMemberTask) error {
	...
	_, err := m.conn.Exec(query, data.Name, data.Growth, data.Integration, data.Type, data.Id)
	return err
}
