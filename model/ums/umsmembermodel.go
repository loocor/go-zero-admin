...
var (
	umsMemberFieldNames          = builderx.RawFieldNames(&UmsMember{})
...
type (
	...
	UmsMember struct {
		...
		LuckyCount            int64     `db:"lucky_count"`            // 剩余抽奖次数
...
func (m *UmsMemberModel) Insert(data UmsMember) (sql.Result, error) {
	...
	ret, err := m.conn.Exec(query, data.MemberLevelId, data.Username, data.Password, data.Nickname, data.Phone, data.Status, data.Icon, data.Gender, data.Birthday, data.City, data.Job, data.PersonalizedSignature, data.SourceType, data.Integration, data.Growth, data.LuckyCount, data.HistoryIntegration)
	return ret, err
}
...
func (m *UmsMemberModel) Update(data UmsMember) error {
	...
	_, err := m.conn.Exec(query, data.MemberLevelId, data.Username, data.Password, data.Nickname, data.Phone, data.Status, data.Icon, data.Gender, data.Birthday, data.City, data.Job, data.PersonalizedSignature, data.SourceType, data.Integration, data.Growth, data.LuckyCount, data.HistoryIntegration, data.Id)
	return err
}
