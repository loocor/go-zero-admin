...
var (
	umsMemberLevelFieldNames          = builderx.RawFieldNames(&UmsMemberLevel{})
...
type (
	...
	UmsMemberLevel struct {
		...
		PriviledgeFreeFreight int64   `db:"privilege_free_freight"` // 是否有免邮特权
		PriviledgeSignIn      int64   `db:"privilege_sign_in"`      // 是否有签到特权
		PriviledgeComment     int64   `db:"privilege_comment"`      // 是否有评论获奖励特权
		PriviledgePromotion   int64   `db:"privilege_promotion"`    // 是否有专享活动特权
		PriviledgeMemberPrice int64   `db:"privilege_member_price"` // 是否有会员价格特权
		PriviledgeBirthday    int64   `db:"privilege_birthday"`     // 是否有生日特权
		Note                  string  `db:"note"`
	}
)
